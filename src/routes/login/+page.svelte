<script lang="ts">
	...
	// SSO detection state
	let ssoProvider = $state<'okta' | 'google' | null>(null);
	...
	async function handleEmailSubmit() {
		...
		try {
			...
			if (response.ok) {
				...
				if (result.hasSso) {
					// Email domain has SSO configured
			...
			// No SSO configured, proceed to password step
			step = 'password';
		} catch (err) {
			// If detection fails, fall back to password login
			step = 'password';
		} finally {
			loading = false;
		}
	...
	function handleSsoLogin() {
		if (!ssoProvider || !teamId) return;
		// Redirect to SSO provider
		window.location.href = `/api/auth/sso/${ssoProvider}?teamId=${teamId}`;
	}
		...
		<div class="w-full max-w-md">
			...
			>
				{#if error}
					<div
						role="alert"
						class="mb-5 flex gap-3 rounded-xl border border-error-200 bg-gradient-to-r from-error-50 to-error-100/50 px-4 py-3.5 dark:border-error-900/50 dark:from-error-950/50 dark:to-error-900/30"
					>
						<svg
						...
						<div>
							...
							<div class="flex items-center gap-2">
								<input
									...
									value={email}
									disabled
									class="flex-1 rounded-xl border border-surface-300 bg-surface-50 px-4 py-3 text-surface-900 dark:border-surface-700 dark:bg-surface-800/50 dark:text-surface-50"
								/>
								<button
									type="button"
									onclick={resetFlow}
									class="btn rounded-xl preset-outlined px-4 py-3 text-sm"
								>
									Change
								</button>
							</div>
				...
				{:else if step === 'sso'}
					...
					<div class="space-y-5">
						...
						<div
							class="rounded-xl border border-primary-200 bg-gradient-to-r from-primary-50 to-primary-100/50 p-4 dark:border-primary-900/50 dark:from-primary-950/50 dark:to-primary-900/30"
						>
							<p class="text-sm text-primary-900 dark:text-primary-100">
								Your organization uses
								<strong class="font-semibold capitalize">{ssoProvider}</strong>
								for sign-in. Click below to continue with your organization's login.
							</p>
						</div>

						<button
							onclick={handleSsoLogin}
							class="btn w-full rounded-xl preset-filled-primary-500 py-3 text-base font-semibold shadow-lg shadow-primary-500/30 transition-all duration-200 hover:shadow-xl hover:shadow-primary-500/40"
						>
							{#if ssoProvider === 'okta'}
								<svg
						...
						<div class="text-center">
							<button
								onclick={resetFlow}
								class="text-sm font-medium text-surface-600 transition-colors hover:text-surface-800 dark:text-surface-400 dark:hover:text-surface-200"
							>
