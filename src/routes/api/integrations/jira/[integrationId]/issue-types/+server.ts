...
/**
 * GET /api/integrations/jira/[integrationId]/issue-types?projectKey=PROJ
 * Get issue types for a Jira project
 */
export const GET: RequestHandler = async (event) => {
	const userId = await requireAuth(event);
	const { integrationId } = event.params;
	const projectKey = event.url.searchParams.get('projectKey');

	if (!projectKey) {
		return json({ error: 'projectKey query parameter required' }, { status: 400 });
	}
	...
	// Get issue types
	const result = await jiraClient.getIssueTypes(projectKey);
	if (result.error) {
		return json({ error: result.error }, { status: 500 });
	}

	return json({ issueTypes: result.data });
};
